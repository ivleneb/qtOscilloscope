    ...
    d_timerId( -1 )
{
    ...
    legendAmpitude = new QwtPlotTextLabel();
    textAmplitude=new QwtText;
    textAmplitude->setText("Amplitud: 100" );
    textAmplitude->setFont(font);
    ...
    textFrecuencia->setText("Frecuencia: 10 Hz");
    textFrecuencia->setFont(font);
...
{
    if ( event->timerId() == d_timerId && !stopped)
    {
        updateCurve();
...
void Plot::showLegendWithCheckbox(int state){
    //qDebug()<<"Enter here";
    if(state==Qt::Checked){
        legendAmpitude->attach(this);
        legendFrequency->attach(this);
    } else{
        legendAmpitude->detach();
        legendFrequency->detach();
    }
    replot();
}

void Plot::changeAmplitudeLab(double val){
    //qDebug()<<"Change label amplitud";
    textAmplitude->setText("Amplitud: "+QString::number(val));
    legendAmpitude->setText(*textAmplitude);
    replot();
}

void Plot::changeFrequencyLab(double val){
    //qDebug()<<"Change label frequency";
    textFrecuencia->setText("Frecuencia: "+QString::number(val)+" Hz");
    legendFrequency->setText(*textFrecuencia);
    replot();

}

void Plot::changeColorCurve(){
    QColor color=QColorDialog::getColor(Qt::yellow, this);
    //qDebug()<<color;
    d_curve->setPen(color);
}

void Plot::changeVerticalShift(double val){
    //qDebug()<<"changeVerticalShift slot";
    this->setAxisScale(QwtPlot::yLeft, -200+val, 200+val );
    QwtPlot::replot();
}

void Plot::stopUpdatingCurve(){
